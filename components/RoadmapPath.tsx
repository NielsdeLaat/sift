...
const SERPENTINE_X = [0.60, 0.28, 0.58, 0.68, 0.36, 0.60, 0.32, 0.68, 0.44, 0.58];
...
export function RoadmapPath({ sections }: Props) {
  ...
  return (
    <div className="relative w-full" style={{ height: totalHeight }}>
      ...
      {/* ── Section badge circles (e.g. "L") ─────────────────── */}
      {badges.map((b) => (
        <div
          key={`badge-${b.label}`}
          className="absolute flex items-center justify-center w-10 h-10 rounded-full border-2 border-primary text-primary font-bold text-sm"
          style={{
            left:      `${b.xFrac * 100}%`,
            top:       b.y,
